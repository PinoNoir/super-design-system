...
The `FileUploader` component allows users to upload files to the system from their local device. It supports two usage patterns: **standalone mode** for simple implementations and **context mode** for advanced status messaging and shared state management.

## Features

- **Dual Usage Modes**: Works independently or with FileContextProvider for status messages
...
### Mode 1: Standalone Usage (Simple)

For basic file uploads without status messages:

```jsx
import React, { useState, useCallback } from 'react';
import { FileUploader, FileCard } from 'sds-ui';
...
### Mode 2: Context Usage (Advanced)

For uploads with status messages and shared state management:

```jsx
import React, { useState, useCallback } from 'react';
import { FileContextProvider, FileUploader, FileCard, StatusMessageDisplay } from 'sds-ui';
...
- **Standalone**: No context provider, no status messages
- **Context Mode**: Wrapped in `FileContextProvider`, includes `StatusMessageDisplay` for upload status feedback
...
The FileUploader supports flexible usage patterns:

### With Context Provider (Recommended)

When wrapped with `FileContextProvider`, you get:

- Centralized state management
- Built-in status messaging via `StatusMessageDisplay`
...
Without context provider, the component:

- Uses internal state management
- Supports controlled mode via `files` and `onFilesChange` props
- Falls back gracefully (no status messages)

```jsx
<FileUploader files={files} onFilesChange={setFiles} onUpload={handleUpload} />
```
